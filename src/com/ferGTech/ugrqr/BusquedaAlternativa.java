...
public class BusquedaAlternativa extends Activity
{
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		if ( !codqr.equals(null) && codqr.length()==11 )
		{
			...
			cEdificio = codqr.substring(3, 5);
			cPlanta = codqr.substring(5, 7);
			...
			if (ene <= 1)
			{
				queTabla = getResources().getString(R.string.tabla_etsiitlugares);
				queTablaDestino = getResources().getString(R.string.tabla_etsiitlugares);
			}
			else if ( ene == 2)
			{
				queTabla = getResources().getString(R.string.tabla_ftilugares);
				queTablaDestino = getResources().getString(R.string.tabla_ftilugares);
			}
	    ...
	    protected Void doInBackground(String... arg) 
	    {
	        ...
	        try 
	        {
	        	...
	        	xconcepto = xconcepto.replace(" ", "%20");
		         textUrl = new URL(getResources().getString(R.string.urlBusquedaAlternativa)+"CONCEPTO="+xconcepto+"&LATABLA="+queTabla);
 ...
 // *******************************************
    ...
    private class BuscaDestinos extends AsyncTask<String, Void, Void>
	{
	    ...
	    protected Void doInBackground(String... arg) 
	    {
	        ...
	        try 
	        {
	        	...
	        	xconcepto = arg[0].replace(" ", "%20");
		         textUrl = new URL(getResources().getString(R.string.urlBusquedaAlternativa)+"CONCEPTO="+xconcepto+"&LATABLA="+queTablaDestino);
	    ...
	    protected void onPostExecute(Void result) 
	    {
	    	...
	    	if (vBr.length >= 1)
	    	{
	    		...
	    		dCod = dCod.replace(" ", "");
	    		String tmp="", tmpplanta="", tmpedificio="", tmpcodqr="";
				tmp = dCod.substring(0, 3);
				dEdificio = dCod.substring(3, 5);
				dPlanta = dCod.substring(5, 7);
				tmpcodqr = dCod.substring(7,11);
    ...
    public void ComparaResultados()
    {
    	...
    	else
    	{
    		...
    		{
    			if ( dPlanta.startsWith("9") )
				{
    				dPlanta.replaceFirst("9", "-1");
				}
    			if ( cPlanta.startsWith("9") )
				{
    				cPlanta.replaceFirst("9", "-1");
				}
